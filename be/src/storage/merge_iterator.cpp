...
inline int compare_chunk(size_t key_columns, const Chunk& lhs, size_t m, const Chunk& rhs, size_t n,
                         const std::string merge_condition, vectorized::Schema _schema) {
...
public:
    explicit ComparableChunk(Chunk* chunk, size_t order, size_t key_columns, string merge_condition,
            ...
            : MergingChunk(chunk), _order(order), _key_columns(key_columns), _merge_condition(merge_condition),
            _schema(_schema) {}
    ...
    bool less_than(size_t lhs_row, const ComparableChunk& rhs) {
        int r = compare_chunk(_key_columns, *_chunk, lhs_row, *rhs._chunk, rhs._compared_row, _merge_condition, _schema);
        return (r < 0) | ((r == 0) & (_order < rhs._order));
    }
...
inline Status HeapMergeIterator::fill(size_t child) {
    ...
    if (st.ok()) {
        ...
        _heap.push(ComparableChunk{chunk, child, _schema.num_key_fields(), merge_condition, _schema});
